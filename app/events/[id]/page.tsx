...
export default async function EventDetailPage({ params, searchParams }: EventDetailPageProps) {
  ...
  const memberDestinationLabel = event.case_type === 'noriai' ? '到着地点（通常は不要）' : '到着地点'
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
              ...
              <details className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <form action={createEventMember} className="mt-4 space-y-5">
                  ...
                  <PlaceSearchSelectInput label={memberDestinationLabel} textName="destinationText" latName="destinationLat" lngName="destinationLng" placeholder="駅名、住所を入力" helperText="入力後に検索を押し、候補から1件選んでください" required={event.case_type === 'sougei'} />
