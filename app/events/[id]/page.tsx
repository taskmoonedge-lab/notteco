      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <form action={deleteEvent}>
            ...
            <PendingSubmitButton
              idleLabel="イベントを削除"
              pendingLabel="削除中..."
              className="inline-flex rounded-2xl border border-red-200 bg-white px-4 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-70"
            />
          </form>
        </div>
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="grid lg:grid-cols-[1.15fr_0.85fr]">
            ...
            <div className="grid grid-cols-2 gap-3 p-8">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4"><p className="text-xs font-medium text-slate-500">車/運転手</p><p className="mt-1 text-lg font-bold text-slate-900">{safeVehicleOffers.length}台/人</p></div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <form action={executePlan}><input type="hidden" name="eventId" value={event.id} /><input type="hidden" name="returnTo" value={adminPath} /><PendingSubmitButton idleLabel="配車する" pendingLabel="配車中..." className="inline-flex items-center justify-center rounded-2xl bg-teal-500 px-5 py-3 text-sm font-bold text-white transition hover:bg-teal-600 disabled:cursor-not-allowed disabled:bg-teal-300" /></form>
              <form action={deleteRoutePlans}><input type="hidden" name="eventId" value={event.id} /><input type="hidden" name="returnTo" value={adminPath} /><PendingSubmitButton idleLabel="配車結果を削除" pendingLabel="削除中..." className="inline-flex items-center justify-center rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-70" /></form>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <details className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4" open>
              ...
              <form action={updateEvent} className="mt-4 space-y-5">
                ...
                <PendingSubmitButton idleLabel="イベント情報を更新する" pendingLabel="イベント情報を更新中..." className="inline-flex w-full items-center justify-center rounded-2xl bg-teal-500 px-4 py-3 text-base font-bold text-white transition hover:bg-teal-600 disabled:cursor-not-allowed disabled:bg-teal-300" />
              </form>
            </details>
          </div>
              ...
              <details className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <form action={createEventMember} className="mt-4 space-y-5">
                  ...
                  <PendingSubmitButton
                    ...
                    className="inline-flex w-full items-center justify-center rounded-2xl bg-teal-500 px-4 py-3 text-base font-bold text-white transition hover:bg-teal-600 disabled:cursor-not-allowed disabled:bg-teal-300"
                  />
                </form>
              </details>
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
              ...
              <details className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <form action={createVehicleOffer} className="mt-4 space-y-5">
                  ...
                  <div><label htmlFor="capacity" className="mb-2 block text-sm font-medium text-slate-700">定員（運転手を除く）</label><input id="capacity" name="capacity" type="number" min="1" max="12" required className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-teal-400 focus:ring-4 focus:ring-teal-100" /></div>
                  <PendingSubmitButton
                    idleLabel="運転手を追加する"
                    pendingLabel="運転手を追加中..."
                    className="inline-flex w-full items-center justify-center rounded-2xl bg-teal-500 px-4 py-3 text-base font-bold text-white transition hover:bg-teal-600 disabled:cursor-not-allowed disabled:bg-teal-300"
                  />
                </form>
              </details>
...
) : null}<label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700"><input type="checkbox" name="canUseRentalCar" defaultChecked={Boolean(member.can_use_rental_car)} className="h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500" />レンタカー参加が可能</label><PendingSubmitButton idleLabel="搭乗者を更新" pendingLabel="搭乗者を更新中..." className="inline-flex w-full items-center justify-center rounded-2xl bg-teal-500 px-3 py-2 text-sm font-bold text-white transition hover:bg-teal-600 disabled:cursor-not-allowed disabled:bg-teal-300" /></form><form action={deleteEventMember}><input type="hidden" name="eventId" value={event.id} /><input type="hidden" name="returnTo" value={adminPath} /><input type="hidden" name="memberId" value={member.id} /><PendingSubmitButton idleLabel="搭乗者を削除" pendingLabel="削除中..." className="inline-flex w-full justify-center rounded-2xl border border-red-200 bg-white px-3 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-70" /></form></div></details>
                    </div>
                      )
                    })()}
                  </li>
                ...
                {safeVehicleOffers.map((vehicle) => (
                  <li key={vehicle.id} className="rounded-2xl border border-slate-200 bg-white p-5">
                    <div className="flex flex-col gap-5">
                      ...
                      <details className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><summary className="cursor-pointer list-none text-sm font-semibold text-slate-900">この運転手を編集</summary><div className="mt-4 space-y-3"><form action={updateVehicleOffer} className="space-y-3"><input type="hidden" name="eventId" value={event.id} /><input type="hidden" name="returnTo" value={adminPath} /><input type="hidden" name="vehicleOfferId" value={vehicle.id} /><div><label className="mb-1 block text-xs font-medium text-slate-700">運転手名</label><input name="driverName" type="text" required defaultValue={vehicle.driver_name} className="w-full rounded-2xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-teal-400 focus:ring-4 focus:ring-teal-100" /></div><PlaceSearchSelectInput label={vehicleStartLabel} textName="startLocationText" latName="startLat" lngName="startLng" defaultText={vehicle.start_location_text} defaultLat={vehicle.start_lat} defaultLng={vehicle.start_lng} placeholder={event.case_type === 'sougei' ? event.destination_text ?? '' : '駅名、住所を入力'} helperText="入力後に検索を押し、候補から1件選んでください" required={event.case_type === 'noriai'} /><div><label className="mb-1 block text-xs font-medium text-slate-700">定員（運転手を除く）</label><input name="capacity" type="number" min="1" max="12" required defaultValue={vehicle.capacity} className="w-full rounded-2xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-teal-400 focus:ring-4 focus:ring-teal-100" /></div><PendingSubmitButton idleLabel="運転手を更新" pendingLabel="運転手を更新中..." className="inline-flex w-full items-center justify-center rounded-2xl bg-teal-500 px-3 py-2 text-sm font-bold text-white transition hover:bg-teal-600 disabled:cursor-not-allowed disabled:bg-teal-300" /></form><form action={deleteVehicleOffer}><input type="hidden" name="eventId" value={event.id} /><input type="hidden" name="returnTo" value={adminPath} /><input type="hidden" name="vehicleOfferId" value={vehicle.id} /><PendingSubmitButton idleLabel="運転手を削除" pendingLabel="削除中..." className="inline-flex w-full justify-center rounded-2xl border border-red-200 bg-white px-3 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-70" /></form></div></details>
                    </div>
                  </li>
                ))}
              </ul>
